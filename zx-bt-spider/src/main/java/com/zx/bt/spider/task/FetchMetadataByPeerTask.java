...
import com.zx.bt.spider.convertor.Map2MetadataConvertor;
...
public class FetchMetadataByPeerTask {
	...
	public FetchMetadataByPeerTask(Config config, BootstrapFactory bootstrapFactory, Bencode bencode,
								   InfoHashRepository infoHashRepository, ObjectMapper objectMapper, MetadataService metadataService) {
		this.config = config;
	...
	public Metadata bytes2Metadata(byte[] bytes, String infoHashHexStr) {
		try {
			...
			return Map2MetadataConvertor.convert(resultMap, objectMapper, infoHashHexStr);
		} catch (Exception e) {
