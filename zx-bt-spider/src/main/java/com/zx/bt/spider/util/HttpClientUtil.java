...
//				throw new RuntimeException(LOG  + "请求失败.当前状态码：" + response.getStatusLine().getStatusCode() +  ",当前路径：" + request.getURI());
	...
	private void initHeader(HttpClientConfig config) {
		...
		/**
		 * 加载  {@link HeaderKey#CHROME_USER_AGENT}
		 */
		headerManager.put(HeaderKey.CHROME_USER_AGENT.getCode(), Collections.singletonList(CHROME_USER_AGENT));
	}
	...
	public enum HeaderKey {
		...
		CHROME_USER_AGENT("chromeUserAgent","只有一个谷歌浏览器的user-agent请求头")
		;
		private String code;
		private String message;
	}
	...
	public class DefaultHttpClientConfig implements HttpClientConfig{
		...
		private List<String> customCookieKeys;
		private Map<String, List<Header>> customHeaders;
	}
}
